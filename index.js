        ...
        let FORCOST_URL = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${api_Key}`,
         ...
         days = [
            ...
            fetch(WEATHERAPI__URL).then(res=>res.json()).then(data=>{
                       //console.log(data);
                       let date = new Date();
                       document.getElementById("main-temp").innerHTML= `${(data.main.temp - 273.15).toFixed()}&degC`;
            ...
            fetch(FORCOST_URL).then(RES=>RES.json()).then(data=>{
               ...
               // console.log(fiveDaysForecost)
                ...
                for(i=1;i<fiveDaysForecost.length;i++){
                    ...
                    nextFiveDays.innerHTML+=`
                       <div class="flex-col justify-center justify-items-center text-center shadow-xl five-din ">
...
let cities = JSON.parse(localStorage.getItem("cities")) || ["Delhi", "Lucknow","Noida","London"];
updateDropdown();
...
function updateDropdown() {
  ...
  // Load weather of first city
  if (cities.length > 0) {
    getCityCoordinate(cities[0]);
}
}
